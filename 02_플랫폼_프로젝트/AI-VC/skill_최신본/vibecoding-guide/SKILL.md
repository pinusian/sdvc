...
이 스킬의 원본(최신 버전)은 `C:\Users\USER\Claude작업용폴더\AI바이브코딩\skill\vibecoding-guide\`에 있다. 이 폴더의 내용을 고치면, `install.bat`을 다시 실행해야 개인 스킬(`~\.claude\skills\vibecoding-guide\`)에 반영된다.
