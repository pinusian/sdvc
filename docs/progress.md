...
> 마지막 업데이트: 2026-09-13(저녁) · 프로젝트: SDVC 웹서비스 · 현재 단계: **매뉴얼·홍보물 산출, [P7-13] PWA 지원, 사용 중 발생한 500(BL-021) 조사·곁가지 3건 수정까지 배포 완료**(프로덕션 `fa73470`) → 다음 **실제 수업 운영**
...
- 완료: **슬라이스 9(출시 전 최소 운영 수단)** — [P8-0]·[P8-1a]·[P8-7a]·[P8-6]·[P8-2]·[P8-3]·[P8-9]
- 완료: **슬라이스 10(교육용 운영)** — [P10-0]·[P8-2b]·[P8-4a]·[P8-2c]·[P10-2a]·[P8-10]
- **운영 맥락(2026-09-12 확정)**: 당분간 **교육용**(수강생 직접 실습), 사업 연계는 **반년~1년 뒤**.
  한국에서 실결제는 **사업자등록이 전제**라 보류하고 Stripe는 테스트 모드로 둔다
- 완료: **[P8-11] 관리자 입구** — `/admin` 고정 주소 + 로그인 화면 + 개발자 화면과 다른 상단 띠. **[P8-12]** 자격 없는 계정으로 오면 맨 404 대신 안내(계정 전환 버튼)
- 완료: **[P8-7] 감사 기록 열람 화면** — 최고관리자 전용, 화면 열람은 기본으로 접힘, 이메일·한국시각으로 표시. 그 과정에서 **[BL-012]**(등급 부여 만료일이 UTC로 저장돼 9시간 밀리던 결함) 발견·수정, 살아있던 계정 1건 정정
- 완료: **[P8-5] 신고 채널 + 접수함** — 개발자 신고 → 운영자 접수함 → 처리 → 신고자에게 답변 표시. **[FR-016] 비상 차단을 실제로 배선**(전에는 함수만 있고 부르는 곳이 없었음, **BL-013**)
- 완료: **[P8-8] 제외 기록 재검증** — "만들어만 두고 안 부른 코드"가 세션 중 3번(BL-008·BL-009·BL-013) 나온 것을 계기로 `tests/lib/dormant.test.ts`(검사로 지키는 제외 기록) 도입. 이 과정에서 **[BL-014]**(프로젝트 개수 한도 미집행)·**[BL-015]**(첨부 파일 미정리) 발견(낮은 우선순위 접수)
- 완료: **[BL-016] 프로필 없는 가입자 2명 발견·복원 + 되풀이 방지**(로그인 시 자가 복구, `ensureAdminRole`보다 먼저 실행되도록 순서 고정)
- 완료: **[P7-12] SC-008 개정** — "재현 테스트 실행 기록"은 정적 사이트에서 지킬 수 없는 말이었다. "말한 변화가 실제 파일에 반영됐는지 서버가 확인한다"로 바꾸고 실제로 구현(BL-001b·BL-005b류 결함을 구조적으로 막음). **P7-2·P7-3(버그 장부)는 폐기** — 대화·버전에 기록이 이미 있고 Clarify 15의 전제(실사용자 다수)가 아직 안 옴
- 완료: **[BL-018] 유지보수 중 배포된 파일을 모델에게 알려준다** — 예전엔 "현재 내용을 붙여넣어 주세요"라고 되물었다. 지금 저장소 내용을 프롬프트에 실어줌(예산 60KB·12개, 넘치면 자르지 않고 이름만)
- 완료: **통합 점검(2026-09-13)** — 가입부터 신고 처리까지 실제 Claude로 전 구간 직접 완주. 그 과정에서 **[BL-019]**(되돌린 뒤 다시 고치면 대화 기록의 옛 상태가 되살아나던 결함, 실제 재현 후 수정·재확인) 발견
- 완료: **[BL-014]** 프로젝트 개수 한도 실제 집행(구현 단계에서 새 프로젝트를 만들려는 순간만 막고, 있는 프로젝트를 고치는 turn은 막지 않음) · **[BL-015]** 프로젝트 삭제 시 첨부 파일도 함께 정리 · **[BL-020]** 감사 화면 중첩 객체 detail(`[object Object]`) 표시 수정 — 접수함에 남은 낮은 우선순위 3건 전부 처리
- 완료: **[P10-4a] 운영 매뉴얼 2종**(상세본 69쪽·간략본 30쪽) · **[P10-4b] 홍보물 2종**(소개자료 16장·기사문 원고 12쪽) — 전부 `docs/`에 있다. 요금 표기는 넣지 않았고(실결제 전), 지어낸 수치도 쓰지 않았다
- 완료: **[P7-13] PWA(설치형 웹앱) 산출물 지원** — 요청했을 때만 매니페스트·서비스 워커를 낸다. 실제 Claude 호출로 검증(서비스 워커 실설치는 도구 제약으로 미확인, 실기기 확인은 사용자 몫)
- 완료: **[BL-021a·b·c]** — `/api/chat` 최상위 try/catch(**실패해도 쓴 글이 남는다**) · `maxDuration=60` 명시 · 현재 파일 병렬 내려받기. **본체 BL-021(500의 원인)은 재현 실패로 '관찰'**
- **다음**: 실제 수업 운영. 사업 전환이 가까워지면 실결제 준비
  - **실결제 준비물**: 한국 Stripe 계정 **신규 생성**(기존 US 계정 국가 변경 불가) · 사업자등록 · 통신판매업 신고 · 도메인 · 이용약관·환불정책. 법률 확인은 전문가에게
  - [P5-2](파일형식·자산 처리)는 [P5-1]에서, [P5-4](목록 화면)는 [P4-4]에서 선행 완료
  - 미결: `/conversations/[id]` 경로를 plan.md 원안대로 `/projects/[id]/chat`으로 옮길지

## 2. 방금 세션에서 한 일 (2026-09-13 저녁, 매뉴얼·홍보물 + PWA + BL-021)

**[P10-4a] 운영 매뉴얼 2종** — 사용자 요청. WBS P10-4의 산출물 중 매뉴얼 부분만 먼저 냈다(전체 테스트 재실행과 출시 승인 G7은 남아 있다).
- **상세본 69쪽**(설계자 보관·유지보수용) / **간략본 30쪽**(사용자 배포용). 사용자가 준 목차 8장을 뼈대로 쓰되 "설치·셋팅을 명확히"라는 요구에 맞춰 2장을 늘렸고, **실제 겪은 결함 BL-001~020을 7.9절에 유형별로** 정리했다. 간략본 부록에는 30분 실습 예시를 넣었다.
- 커밋 `2da6047`. 파일은 `docs/`에 있다.

**[P10-4b] 홍보물 2종** — 대상은 **이용자+투자자 겸용**, 요금은 빼고 "교육 과정 제공"으로(사용자 확인).
- **소개자료 PPT 16장** + **홍보 원고 12쪽**(보도자료·특집 기사문·채널별 짧은 소개문·배포 전 채울 것).
- 요청받은 네 축(시간·효율·정확성·유지보수)을 기존 바이브 코딩과의 좌우 비교표로 놓고 한 축씩 폈다.
- **수치를 지어내지 않았다.** "○배 빨라진다"류는 측정한 적이 없으므로 구조적 사실과 저장소에서 확인 가능한 값(작업 71건·단위 검사 700여 건·e2e 18건·결함 20건)만 썼고, 원고 4부에 **수치별 근거 표**를 달았다. 사람 이름·기관명·연락처는 대괄호로 비워 뒀다. 커밋 `f64fe40`.

**[P7-13] PWA(설치형 웹앱) 산출물 지원** — 홍보자료 검토 중 사용자가 "휴대폰 앱이 왜 안 되나"를 물은 데서 시작. 셋 중 **이것만 즉시 가능**했다 — `ALLOWED_EXTENSIONS`에 `webmanifest`가 **이미 있었고** 서비스 워커도 평범한 `.js`라 서빙 라우트를 한 글자도 안 고쳐도 됐다. 빠진 것은 프롬프트 지시문 하나뿐.
- 요청했을 때만 만들고, 아이콘은 SVG로만(모델이 그림 파일을 못 만든다는 한계를 사용자에게 그대로 알리게), "네이티브 앱이 되는 것은 아니다"를 못 박았다.
- **실제 Claude 호출로 검증**: "휴대폰에 앱처럼 설치되게 해주세요" → `manifest.webmanifest`·`icon.svg`·`sw.js`·수정된 `index.html` 4개를 지시대로 냈고, 아이콘 한계와 "앱스토어 앱이 아니다"를 **모델이 스스로 답변에 포함**했다. 파일이 올바른 content-type으로 실제 서빙되는 것까지 확인.
- **서비스 워커 실설치·오프라인은 확인 못 했다** — 이 브라우저 자동화 도구가 서비스 워커 등록 자체를 지원하지 않는다(Next.js **자체 정적 자산**으로 대조군을 만들어 우리 코드 결함이 아님을 확인). 실기기 확인은 사용자 몫으로 남김. 커밋 `9223182`→`3a2c51b`.

**명세 단계에 가드레일이 없다는 것을 실증** — 사용자가 "정적 사이트로 안 되는 1·2번을 개발자가 요구하면 어떻게 되나"를 물어 **실제로 해봤다**. 블록 1에서 "회원가입/로그인 + 게시판 + 참가비 결제"를 요청하니 AI가 **아무 경고 없이 받아들이고** "회원가입하면 DB에 사용자가 생긴다"를 TDD 예시로 들며 진행했다. 프롬프트 어디에도 "정적 사이트만 만든다"는 문구가 없다(`prompt.ts` 전수 확인). **고치자고 제안했으나 사용자가 다른 질문으로 넘어가 미착수** — §4에 남김.

**[BL-021] 사용 중 500 발생** — 사용자가 빵집 프로젝트에서 "이어서 수정"으로 회원가입·운영자화면·문의게시판 추가를 요청하자 `요청에 실패했습니다. (상태 500)`.
- **조사로 좁힌 것**: ①제 PWA 변경과 무관(당시 프로덕션은 `b59a40a`) ②**메시지가 저장되지도, Claude가 불리지도 않았다**(그 시각 이후 `messages` 0건·`usage_logs` 0건) → `appendMessage` **이전**에 죽었다 ③500 본문에 JSON이 없었다 → 우리 라우트 응답이 아니라 **플랫폼 500**.
- **재현 실패**: 같은 프로젝트·대화를 통째로 복제(파일 16개·메시지 34건·`current_block=done`)해 로컬·프로덕션에서 같은 문장으로 재시도했으나 **둘 다 정상 동작**. 원인 미상으로 '관찰' 유지.
- **대신 구조적 결함 3건을 찾아 고쳤다**: `/api/chat`에 최상위 try/catch가 없어 **쓴 글까지 사라지던 것**(BL-021a), `maxDuration`이 없어 플랫폼 기본값에 맡겨져 있던 것(BL-021b — 프로덕션에서 복잡한 요청이 첫 글자까지 **85초를 사고만 한 것** 실측), 현재 파일을 순차로 내려받아 스트림이 늦게 열리던 것(BL-021c — 파일 11개에 **5.8초** 실측). 커밋 `8bb98d8`→`fa73470`.

**배포**: 두 저장소 모두 푸시. `/api/health`의 `commit`이 로컬 HEAD(`fa73470`)와 일치하는 것으로 반영 확인.

**검증**: lint 0 / 단위 **72파일·740건**(파일 수 대조) / 클린 빌드.

> 이번 세션의 교훈 하나: **문서·장표도 "만들었다"와 "제대로 보인다"는 다르다.** 매뉴얼과 PPT 모두 실제로 렌더해 본 뒤에야 결함이 드러났다(번호 목록이 절을 넘어 이어지던 것, 없는 글꼴을 지정해 코드가 본문과 구별 안 되던 것, 모든 장표 아래 1.3인치 빈 띠). 코드에서 겪은 것과 같은 부류다.

## 2-1. 그 앞 세션 (2026-09-13, Phase 7·8 마무리 + 통합 점검)

- **[BL-008 후속 확인]** 서버관리자 자동 승격이 실제로 됐는지 DB로 직접 확인 — `role: admin` 정상. 사용자가 "체험 등급"이라고 본 것은 **등급(grade) 배지**였지 역할(role)이 아니었다(관리자도 자기 프로젝트에선 체험 등급이 맞다).
- **[P8-11] 관리자 입구** — 승격은 됐는데 **들어갈 길이 없었다**(BL-009: `/admin` 링크가 코드 어디에도 없었고, `/admin`은 `/login`으로 튕겼다). `lib/admin/entry.ts`에 판정을 한 곳으로 모으고, 반전된 상단 띠로 개발자 화면과 구별.
- **[P8-12]** 위 설계를 사용자가 실사용 중 다시 걸렸다(BL-010: 크롬에서 `/admin`이 404 — 다른 계정으로 로그인돼 있었을 뿐인데 화면이 아무 말도 안 함). 세 갈래(로그인/콘솔/404)를 두 갈래로 줄이고, 자격 없는 계정이면 "지금 누구인지" 안내 + 계정 전환 버튼.
- **[P8-7] 감사 기록 열람 화면** — 이번 세션에만 DB를 직접 세 번 열어 확인했던 것을 화면으로 대체. 실물 8건을 먼저 읽고 설계(화면 열람은 접힘, 이메일·한국시각 변환). 정리 중 **[BL-012]** 발견: 등급 부여 만료일이 UTC 자정으로 저장돼 실제 만료가 9시간 늦었다 — `lib/time/seoul.ts`로 한국 시각 계산을 한 곳에 모으고, 살아있던 계정 1건 정정(감사 로그에 정정 사실도 남김).
- **[P8-5] 신고 채널 + 접수함** — Clarify 17에서 "보내는 쪽과 보는 쪽을 함께 만들기"로 미뤄둔 것을 이제 냄. 만들다가 **[BL-013]** 발견: `setProjectBlocked`(FR-016 비상 차단)를 아무도 안 부르고 있었다 — 접수함에서 신고 처리와 함께 가리도록 배선.
- **[P8-8] 제외 기록 재검증** — "만들어만 두고 안 부른 코드"가 세션 중 세 번(BL-008·BL-009·BL-013) 나온 것을 계기로, 글로 된 기록 대신 **검사**(`tests/lib/dormant.test.ts`)로 지키게 함. 훑다가 **[BL-014]**(프로젝트 개수 한도 미집행)·**[BL-015]**(첨부 파일 미정리) 발견 — 둘 다 낮은 우선순위로 접수만.
- **[BL-016]** 위 훑기 도중 가입자 2명(`lee.myungho@…`·`parkbc@kw.ac.kr`)에게 `profiles` 행이 없어 **실제로 서비스를 못 쓰고 있었던 것**을 발견 — 트리거 이전 가입으로 추정. 즉시 복원(가입 시각 기준 체험 7일)하고, 로그인 시 자가 복구를 배선(`ensureAdminRole`보다 **먼저** 실행해야 함 — update는 없는 행을 못 고친다).
- **[P7-12] SC-008 개정** — 사용자에게 "P7-2·P7-3(버그 장부)이 지금도 필요한가"를 먼저 물어 **폐기**로 정리(대화·버전에 이미 기록이 있고, Clarify 15의 전제인 "실사용자가 생겨 잊는 문제"가 아직 안 왔다). 대신 거기 딸려 있던 SC-008("재현 테스트 실행 기록")은 정적 사이트에서 지킬 수 없는 말이었으므로 "말한 변화가 실제 파일에 반영됐는지 서버가 확인한다"로 바꿔 구현 — BL-001b·BL-005b류를 구조적으로 막는다.
- **[BL-018]** 위 작업 e2e에서 모델이 "현재 내용을 붙여넣어 주시겠어요?"라고 되묻는 것을 봄 — 유지보수 지시문이 실제 배포 파일을 안 주고 있었다. `loadCurrentFiles`로 지금 저장소 내용을 프롬프트에 실어줌(예산 60KB·12개, 넘치면 자르지 않고 이름만 — 반쪽을 보여주면 모델이 반쪽으로 다시 낸다).
- **통합 점검**: 가입부터 신고 처리까지 수강생 한 명 몫을 실제 Claude로 처음부터 끝까지 직접 밟음(자동화는 이 컴퓨터의 메모리 부족으로 멈춰 직접 재현 후 수동 진행으로 전환 — 서버는 항상 빨랐다, §6 참고). 그 과정에서 **[BL-019]** 발견: 되돌리기 직후 실제 파일은 옳은데(BL-018 정상), 이어서 사소한 걸 고치면 **대화 기록의 되돌리기 이전 상태가 되살아났다** — BL-018의 "지금 파일이 기록보다 정확하다"는 한 줄 안내보다 여러 턴의 대화가 모델에게 더 강하게 작용한 것. 되돌린 사실을 시스템 프롬프트가 아니라 **대화의 한 턴(assistant 메시지)**으로 남기도록 고침. 같은 상황을 실제로 재현→수정→재확인.
- 통합 점검 중 **[BL-020]**(감사 화면 자기 필터가 `[object Object]`로 표시) 발견, 낮은 우선순위 접수 → **같은 세션에서 세 건 모두 처리**: [BL-014] 구현 단계에서 새 프로젝트를 만들려는 순간만 한도를 보고 Claude를 부르지 않고 막음(있는 프로젝트 고치기는 그대로 동작) · [BL-015] 프로젝트 삭제 시 그 대화의 첨부까지 함께 지움(발행 전 첨부는 범위 밖으로 남김) · [BL-020] 중첩 객체 detail을 재귀적으로 펼쳐 `[object Object]` 제거. 셋 다 `dormant.test.ts`의 KNOWN_DORMANT에서 자동으로 빠짐(검사가 스스로 확인).
- 검증: lint 0 / 단위 700건(71파일, 파일 수까지 대조) / 클린 빌드 / e2e 다수. **BL-017(vitest가 가끔 일부만 수집하고 통과로 보고)이 이번에도 재현** — 재실행하면 정상.

## 2-2. 그 앞 세션 (2026-09-11, 슬라이스 4 완결 → **MVP 완료**)
...
## 2-3. 그 앞 세션 (2026-09-11 새벽, 슬라이스 3)
...
### 슬라이스 10 — 교육용 운영 실제 실행 (2026-09-12)

운영자 1명 + **체험이 끝나 막힌** 수강생 1명으로 한 바퀴.

| 확인한 것 | 결과 |
|---|---|
| 처음엔 402 `trial_expired`로 막힘 | 통과 |
| 화면에서 기본 등급을 **기간·사유와 함께** 부여 → DB 기록 | 통과 |
| **막혔던 수강생이 대화 200** | 통과 |
| 계정 한도를 **0**으로 → 402 `token_limit` (부여받았어도 막힘) | 통과 |
| 한도를 비우면 등급 기본값으로 복구 | 통과 |
| 만료일이 지나자 **즉시** 402 (정리 작업을 기다리지 않음) | 통과 |
| **결제 중인 사람은 부여와 무관하게 통과** (강등되지 않음) | 통과 |
| 감사 로그에 정책 변경 3건, 무슨 등급을 줬는지까지 | 통과 |
| `/privacy` 200, Anthropic 전달 사실 명시, 가입 화면에서 링크 | 통과 |

### 슬라이스 9 — 출시 전 최소 운영 수단 (2026-09-12, **완료**)
...
| [P8-2] 개발자 관리 화면 | ✅ |
| [P8-3] 사용량·원가 대시보드 | ✅ |
| [P8-9] 슬라이스 9 검증 | ✅ **실패 0건** |

**[P8-9] 실제 실행 결과** (관리자 1명 + 개발자 1명 + 공개 홈페이지로):

| 확인한 것 | 결과 |
|---|---|
| 개발자가 관리 API를 부르면 **404**(403 아님 — 화면의 존재를 숨긴다) | 통과 |
| 관리자는 `/admin`이 열리고 목록·수지가 보인다 | 통과 |
| 산출물 비상 차단 → `/site`가 **404**, 풀면 원래대로 | 통과 |
| 화면에서 사유를 적어 정지 → DB에 시각·사유 기록 | 통과 |
| 정지된 개발자: **로그인은 유지**, 대화 **402**(`suspended`, 이유 안내), 홈페이지 **404** | 통과 |
| 정지 해제 → 홈페이지·대화 모두 복구 | 통과 |
| 감사 로그에 **열람·정지·거부된 시도**까지 남음 | 통과 |
| 지원 등급의 정지 시도 → **403**, 그 시도도 로그에 | 통과 |
...
- [x] **[P10-0] 마이그레이션 0009** — `granted_grade`·`granted_until`·`granted_reason`·`monthly_token_limit`
- [x] **[P8-2b] 등급 부여 + [P8-4a] 계정별 한도** — `effectiveGrade`: **결제 active → 결제 등급 / 살아있는 부여 → 부여 등급 / 아니면 원래 등급.** 만료는 **읽을 때** 판정(정리 작업을 기다리면 하루 더 공짜). 한도는 `??`가 아니라 `!= null`로 판정(**0은 완전 차단**, null은 등급 기본값)
- [x] **[P8-2c] 관리 화면** — 부여·한도는 `policy:change`라 **최고관리자만**. 기존 관문·감사 로그를 그대로 탄다
- [x] **[P10-2a] 개인정보 처리방침** — `/privacy` + 가입 화면 링크. 커밋 `d0463bd`
- [x] **[P8-10] 검증** — 위 §3 표. 실패 0건

### 슬라이스 9(출시 전 최소 운영 수단) — 완료
...
- [x] **[P8-2] 개발자 관리 + [P8-3] 원가 대시보드** — `/admin` 한 화면. **판정·행위·감사기록 세 가지를 `requireAdmin` 한 관문에 묶었다**(라우트마다 따로 쓰면 언젠가 하나를 빠뜨린다). 정지에는 **사유를 반드시** 적게 한다. 지난 체험은 **오늘부터** 다시 센다(과거에 더하면 여전히 만료다). 커밋 `bc72f29`
  - REFACTOR: 등급별 요금($12/$35)이 요금제 화면과 원가 계산 두 곳에 있었다 → `plans.ts`의 `GRADE_PRICE_USD` 하나로. 어긋나면 마진이 거짓말을 한다
  - **TDD 이탈 1건 기록**: `/api/admin/usage`는 테스트와 구현을 같은 단계에서 써서 RED를 관찰하지 못했다. 나머지는 RED 확인함
- [x] **[P8-9] 슬라이스 9 검증** — 위 §3 표. 실패 0건
...
- **사용자 결정 대기 — 명세 단계 가드레일.** 블록 1에서 정적 사이트로 **불가능한 기능**(회원가입·게시판·예약·결제)을 요구해도 AI가 아무 경고 없이 받아들여 명세·계획·작업분해까지 짜고, **구현 단계에 가서야** 막힌다(2026-09-13 실증). 가장 비싼 지점에서 실망하는 구조다. 고치는 방법은 PWA와 같은 **프롬프트 지시문 한 절**이라 싸다(1·2번 기능을 실제로 만드는 8~24세션과는 완전히 다른 얘기). 제안했으나 착수 승인 전.
- **관찰 중 — [BL-021]** 사용 중 발생한 500. 재현 실패로 원인 미상. 다만 이제 맨 500이 아니라 우리 문구가 뜨고 입력도 남으므로, 재발하면 증상을 더 정확히 받을 수 있다.
- 접수함의 다른 관찰 1건(BL-017, vitest가 드물게 파일을 부분 수집하고도 통과로 보고. 대응책은 §6에 있음).
- 실결제 전환은 사용자 쪽 준비물이 있다(§1 참고 — 한국 Stripe 계정 신규 가입, 사업자등록 등). 급하지 않다.
- **MFA(2단계 인증)는 의도적으로 건너뛴다.** 관리자가 한 명뿐인 지금 켜면 휴대폰을 잃는 순간 운영 콘솔에 아무도 못 들어간다. 복구 수단(백업 코드·두 번째 관리자)을 같이 설계할 때 다시 본다.
...
- **감사 화면·프롬프트에서 UTC/한국시각을 섞어 쓰면 반드시 어긋난다.** Vercel은 UTC로 돈다. 한국 시각 계산은 한 파일(`lib/time/seoul.ts`)에만 두고 그걸로만 쓸 것 — 등급 부여 만료일이 9시간 밀렸던 BL-012가 이 흩어짐 때문이었다.
- **"만들어만 두고 아무도 안 부르는 함수"는 단위 테스트로는 절대 안 잡힌다.** 단위 테스트는 함수를 직접 부르므로 통과한다. `ensureAdminRole`(BL-008)·관리자 입구(BL-009)·`setProjectBlocked`(BL-013)가 전부 이 유형이었다 — `tests/lib/dormant.test.ts`로 `src/lib`의 export 함수를 훑어 새로 잠든 함수를 검사로 잡는다.
- **되돌리기(rollback) 뒤의 대화는 특별 취급이 필요하다.** "지금 파일이 대화 기록보다 정확하다"는 시스템 프롬프트 각주만으로는 부족했다 — 여러 턴의 대화 자체가 그 한 줄보다 모델에게 더 강하게 작용해 옛 상태가 되살아났다(BL-019). 되돌린 사실은 **대화의 한 턴(assistant 메시지)**으로 남겨야 한다.
- **`vitest run`이 가끔 파일 일부만 수집하고도 "통과"로 보고한다**(BL-017, 이 세션에서 두 번 재현). 원인 미상, 낮은 여유 메모리와 상관관계 있어 보임. **"통과" 여부만 보지 말고 디스크 파일 수와 수집된 파일 수를 항상 대조할 것.** 다르면 그냥 재실행.
- **이 컴퓨터는 크롬 탭이 쌓이면 여유 메모리가 1GB대로 떨어질 수 있다.** 그 상태에서 Playwright가 새 헤드리스 브라우저를 띄우면 서버·API는 멀쩡한데 자동화만 사실상 멈춘다(수십 분). 오래 걸리면 먼저 여유 메모리를 확인하고, 급하면 이미 떠 있는 가벼운 브라우저 도구로 같은 서버에 직접 접속해 재현해볼 것 — 값싸게 "제품 문제 vs 환경 문제"를 가른다.
- **이 브라우저 도구의 탭들은 쿠키(세션)를 공유한다.** 한 탭에서 다른 계정으로 로그인하면 다른 탭들도 그 계정으로 바뀐다 — 학생·운영자 두 역할을 같이 확인할 땐 이 점을 염두에 둘 것(계정 전환 직후엔 DB로 직접 확인하는 편이 안전하다).
- **`.claude/launch.json`의 `preview_start`(이름 지정)는 여전히 이 PC에서 실패한다** — `npm.cmd` 경로 공백 문제, 세션이 바뀌어도 그대로다. dev 서버는 `nohup npm run dev &` 형태로 직접 띄우고 `preview_start`는 `url`로만 붙일 것.
- **작업 중 `rm -rf .next && npm run build`를 돌리면 떠 있던 dev 서버가 죽는다.** 검증 뒤엔 dev 서버를 다시 띄워야 한다(이번에도 한 번 이걸로 자동화 재현이 헷갈렸다).
- **이 브라우저 도구는 서비스 워커를 등록하지 못한다.** `navigator.serviceWorker.register()`가 어떤 자원에 대해서도 "unknown error"로 실패한다 — Next.js가 직접 서빙하는 **자체 정적 청크 파일**로 대조군을 만들어 확인했다(2026-09-13, [P7-13]). 우리 코드 문제로 오해하지 말 것. PWA 실설치·오프라인 확인은 실기기나 실제 크롬에서 해야 한다.
- **`curl`은 이 샌드박스에서 외부 도메인에 막힌다**(exit 7). 프로덕션 확인은 `node -e "fetch(...)"`로. localhost는 `curl`도 된다.
- **스크립트에서 `@supabase/supabase-js`를 쓰려면 파일이 `SDVC-app/` 안에 있어야 한다** — Node의 모듈 해석은 실행 위치(cwd)가 아니라 **파일 위치**를 기준으로 위로 올라가며 `node_modules`를 찾는다. 스크래치 폴더에 두면 `ERR_MODULE_NOT_FOUND`. 임시 스크립트는 앱 폴더에 점(`.`)으로 시작하는 이름으로 두고 쓰고 나서 지운다.
- **문서·장표도 반드시 렌더해서 눈으로 볼 것.** "파일이 만들어졌다"와 "제대로 보인다"는 다르다. 이 PC에는 LibreOffice가 없지만 **Word·PowerPoint COM이 있어** PowerShell로 열어 쪽수 확인·목차 갱신·PDF 변환이 된다(`scratchpad/manual/finalize.ps1` 방식). PDF는 `pymupdf`로 이미지화해 확인한다. 이 절차로 매뉴얼 3건·PPT 1건의 결함을 잡았다.
- **`docx`/`pptx` 산출물의 고정폭 글꼴은 `Consolas`를 쓴다.** `D2Coding` 등 없는 글꼴을 지정하면 조용히 세리프로 대체돼 코드가 본문과 구별되지 않는다. 그리고 **배포용 한국어 문구는 고정폭으로 쓰지 말 것**(자간이 벌어져 읽기 나쁘다).
