...
> 마지막 업데이트: 2026-09-11 · 프로젝트: SDVC 웹서비스 · 현재 단계: **[P5-1] 완료 — `/site/{주소}` 서빙 작동(프로덕션 검증)** → 다음 **[P5-3] 공개범위 변경 UI**
...
## 2. 방금 세션에서 한 일 (2026-09-11 늦은밤, 슬라이스 4 착수)

- **[P4-6]**(사용자 신고 대응) + **[P5-1]** 완료. 상세는 §4.
- 사용자가 직접 만든 홈페이지(`site-qiioxo`, "브레드앤버터 베이커리")로 실제 테스트 — 파일 9개가 온전히 저장돼 있고 index.html이 참조하는 4개 파일 모두 존재함을 확인. **비공개라서 주인이 로그인해야 보인다**고 안내함.
...
- [ ] **[P5-2]** 공개범위 판단 세부(검색 노출 등) · [ ] **[P5-3]** 공개범위 변경 UI · [ ] **[P5-4]** 슬라이스 검증

## 5. 막힌 것 / 사용자 결정 대기

- 없음. 다음은 **[P5-3] 공개범위 변경 UI** — 대시보드에서 비공개↔링크공개↔전체공개를 바꾸는 기능. 이게 있어야 만든 홈페이지를 남에게 보여줄 수 있다(지금은 주인만 볼 수 있는 상태). 그 다음 [P5-2] 세부·[P5-4] 검증으로 **MVP 완료**.
