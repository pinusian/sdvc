...
## 12. 슬라이스 10 설계 — 교육용 운영 (2026-09-12, FR-035·036·037)

**맥락**: 당분간 교육용(수강생이 직접 실습), 사업 연계는 반년~1년 뒤.
실결제는 사업자등록이 전제라 **보류**하고, 수강생을 실제로 받을 수 있게 하는 것만 만든다.

### 12-1. 지금 무엇이 막히나

| 막히는 것 | 왜 |
|---|---|
| 수강생이 7일·50만 토큰에서 멈춘다 | 등급은 **Stripe 결제로만** 올라간다 |
| 한 명이 폭주해도 손쓸 수 없다 | 한도가 등급별로 **코드에 고정**되어 있다 |

### 12-2. 등급 부여 (FR-035)

**결제와 뒤섞지 않는다.** `grade`는 지금처럼 결제(웹훅)가 관리하고, 부여는 **따로 둔다**:

```
profiles.granted_grade   basic | pro | null   ← 운영자가 준 등급
profiles.granted_until   timestamptz | null   ← 언제까지 (Clarify 23)
```

**실효 등급**을 정하는 순서 — 섞이면 "결제했는데 강등"·"공짜로 프로"가 생긴다:

1. 구독이 `active`면 → **결제한 등급**(부여를 무시한다. 돈을 낸 쪽이 우선)
2. 아니고 부여가 살아 있으면 → **부여한 등급**, 체험 만료 검사도 건너뛴다
3. 둘 다 아니면 → 지금까지와 같다(체험 기간·한도 판정)

만료는 **읽을 때 판정**한다(지난 부여는 없는 것으로 본다). 하루 한 번 도는 정리 작업이
지난 값을 지우기는 하되, 그것을 기다리지 않는다 — cron이 하루 밀리면 하루 더 공짜가 된다.

### 12-3. 계정별 한도 (FR-036)

```
profiles.monthly_token_limit   integer | null   ← 비우면 등급 기본값
```

`canStartChat`이 `GRADE_LIMITS[grade].monthlyTokens` 대신 이 값을 먼저 본다.
**0도 유효한 값이다**(완전히 막기) — `null`과 구별해야 하므로 `?? `가 아니라 `!= null`로 판정한다.

### 12-4. 개인정보 처리방침 (FR-037)

`/privacy` 한 장 + 가입 화면 링크. 무엇을 받고(이메일)·왜(계정 식별)·어디에 두고(Supabase)·
언제 지우는지(탈퇴 시, 유예 후). **이용약관·환불정책은 만들지 않는다** — 돈을 받지 않는 동안은
약속할 것이 없고, 사업 전환 때 실제 조건에 맞춰 쓰는 편이 낫다.

### 12-5. 이 보완이 건드리는 것

| 구분 | 내용 |
|---|---|
| DB | 마이그레이션 `0009` — `granted_grade`·`granted_until`·`monthly_token_limit` |
| 판정 | `loadAccountState`가 실효 등급·한도를 계산해서 넘긴다 (판정 로직은 한 곳에 유지) |
| API | `/api/admin/developers`에 `grant_grade`·`set_limit` 행위 추가 (기존 관문·감사 로그 그대로 탄다) |
| 화면 | `/admin` 목록에 등급 부여·한도 지정, 가입 화면에 처방침 링크, `/privacy` 신설 |
| 정리 작업 | 지난 부여를 비운다(읽을 때 판정이 우선이고, 이건 청소일 뿐) |

### 12-6. 헌장 점검

| 헌장 규칙 | 이번 보완에서 |
|---|---|
| TDD 필수 | 전부 RED→GREEN. **"결제한 사람이 부여 때문에 강등되지 않는가"**·**"지난 부여가 즉시 무효인가"** 부터 |
| 보안 규칙 | 부여·한도 변경은 관리자 관문(`requireAdmin`)과 감사 로그를 그대로 탄다. **수강생 본인은 못 바꾼다**(Clarify 22) |
| 기록 의무 | 누가 누구에게 무슨 등급을 언제까지 줬는지 감사 로그에 남는다 |
| 완료 보고 규칙 | 실제로 **부여해 한도가 늘어나고, 만료일이 지나면 도로 막히는 것**까지 확인 |
