...
const doc = new Document({
  ...
  sections: [{
    ...
    children: [
      ...
      p(''),
      p('예상 기간 — 하루 8시간 풀가동 기준 (v2.1 갱신, 2026-09-08)', { bold: true }),
      p('1세션(1~2시간, 평균 1.5시간) × 하루 8시간 = 1일 약 5.3세션 처리. "이론치"는 순수 계산값, "보정치"는 검증·디버깅 시간(+) 과 연속 작업으로 줄어드는 맥락 재적재 비용(–)을 함께 반영해 +25% 버퍼를 더한 값이다.', { after: 140 }),
      simpleTable(
        ['마일스톤', '세션', '이론 근무일', '보정 근무일 (+25%)', '캘린더(주5일)', '캘린더(매일)'],
        [
          ['MVP 완성', '36.5', '약 7일', '약 8.5~9일', '약 2주', '약 1.5주'],
          ['판매 가능', '45.5', '약 8.5일', '약 10.5~11일', '약 2.5주', '약 1.5주'],
          ['전체 완료', '75.5', '약 14일', '약 17.5~18일', '약 3.5주', '약 2.5주'],
        ],
        [2478, 1600, 2600, 3200, 2400, 2400]
      ),
      p(''),
      p('※ 근무일수는 "보정 근무일" 기준으로 보는 것을 권장한다. Phase 3(엔진 이식)·Phase 7(개발자 모드)이 가장 불확실해 이보다 늘어날 수 있다.', { italics: true, color: '595959' }),
      p('※ 풀가동으로도 못 줄이는 지점 — Stripe 사업자 심사(실제 카드결제 승인, 보통 수 영업일 소요, 작업속도와 무관하게 대기)는 P6-8 검증(테스트 모드로 가능)을 막지 않지만, 진짜 결제를 받기 시작하는 시점은 이 심사 완료 이후다. 도메인 전파(수분~48시간)도 마찬가지로 작업과 병행되는 대기시간이다.', { italics: true, color: 'C00000' }),
      p('※ 규모를 줄이고 싶다면 Phase 9를 출시 후로 미루는 것이 가장 안전한 절단면이다(70.5 → 68.5세션).', { italics: true, color: '595959' }),
      new Paragraph({ children: [new PageBreak()] }),
