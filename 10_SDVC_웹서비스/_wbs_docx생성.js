...
const P7 = [
  ...
  { type: 'task', isNew: N, c: ['P7-2', '버그 등록·추적 (내 프로젝트)', '개발자가 자기 프로젝트의 문제를 자연어 프롬프트로 설명해 등록한다(형식을 갖춘 양식이 아니라 대화하듯 입력). 접수 이후 상태(접수→수정중→완료)는 자동으로 추적된다. 제작 당시와 제작 후 유지보수 모두에 사용.', 'Claude', '버그 등록·추적 기능', '1.5'] },
...
const P8 = [
  ...
  { type: 'task', isNew: N, c: ['P8-8', '[제외 결정] SDVC 플랫폼 유지보수', 'SDVC 자체의 버그 수정·기능 보완은 웹 관리화면으로 만들지 않는다. 서버관리자가 발견했거나 P8-5로 접수된 플랫폼 문제를 내 PC의 Claude Code에 프롬프트로 설명하면 Claude Code가 직접 코드를 고친다. 그 판단 근거와 대체 절차를 문서로 남긴다. → 웹 개발 범위에서 제외하여 기간·비용을 절감.', '공동', '제외 결정 기록', '0.5'] },
];
    ...
    children: [
      ...
      simpleTable(
        ...
        [
          ...
          ['서버관리자의 "시스템 관리"', '플랫폼 관리', '서버관리자가 SDVC 자체를 관리하는 것. 서버관리자가 자기 프로젝트를 만들려면 개발자로 겸직 등록해야 한다(P9-4)'],
          ['버그 등록', '① 프로젝트 버그 / ② 플랫폼 신고', '둘 다 프롬프트로 입력하지만 처리 엔진이 다르다 : ① 내 산출물의 문제(P7-2) → SDVC 웹 엔진(P7-3) ② SDVC 자체의 문제(P7-5) → 내 PC Claude Code(P8-8)'],
        ],
        [3200, 3200, 8278]
      ),
      p(''),
      ...
      bullet('"시스템"의 두 가지 뜻(플랫폼 / 산출물)을 용어로 분리했다 — 2.2절. 서버관리자는 플랫폼(SDVC 자체)을, 개발자는 자신의 프로젝트를 유지보수한다. 서버관리자가 자기 프로젝트(예 : 개인 홈페이지)를 만들고 싶으면 별도로 개발자 등록을 겸해야 하며, 이는 P9-4(역할 겸직)로 처리된다 — "서버관리자 자격"으로 프로젝트를 유지보수하는 경로는 두지 않는다.'),
      bullet('"버그 등록"의 대상과 처리 엔진을 분리했다 — 두 계층 모두 프롬프트를 입력하면 AI가 직접 코드를 고치는 방식은 같지만, 처리 엔진이 다르다. 개발자의 버그는 SDVC 웹 엔진(P7-2→P7-3)이, 서버관리자가 발견한 플랫폼 버그는 내 PC의 Claude Code(P8-8, 웹 범위 밖)가 처리한다. 창구를 하나로 만들면 어느 엔진이 고쳐야 할 문제인지 구분되지 않는다.'),
...
Packer.toBuffer(doc).then((buf) => {
  ...
  L.push('"버그"는 둘 다 프롬프트 입력 방식이지만 처리 엔진이 다르다: ① 프로젝트 버그(P7-2, SDVC 웹 엔진이 고침) ② 플랫폼 신고(P7-5 → P8-5 접수 → 내 PC Claude Code가 고침, P8-8).');
  L.push('서버관리자가 자기 프로젝트를 만들려면 개발자로 겸직 등록해야 한다(P9-4) — "서버관리자 자격"으로 프로젝트를 유지보수하는 경로는 없다.');
  L.push('');
